...
public class ColorWheel extends SubsystemBase {
  ...
  public ColorWheel() {
    ...
    spinnerPID.setOutputRange(Constants.COLORWHEEL_MIN_OUTPUT,
        Constants.COLORWHEEL_MAX_OUTPUT_RAISE);

    spinnerMotor.setSmartCurrentLimit(Constants.COLORWHEEL_MAX_CURRENT_STALL,
        Constants.COLORWHEEL_MAX_CURRENT_RUN);


    // for the Neo 550 motor built in encoder we need to do the external gear reductions math in the
    // setPositionConversionFactor
    // 8 to 1 for the drive wheel to color wheel and 10 to 1 for the for the gearbox on the motor.
    // 80 motor rotations = 1 color wheel rotation
  ...
  public void rotateColorWheel(double rotations) {
    if (spinnerEncoder.getPosition() < Constants.SPINNER_MAX_HEIGHT - 2) // The -2 accounts for any
                                                                         // small error in the
                                                                         // encoder value
    {
      spinnerPID.setOutputRange(Constants.COLORWHEEL_MIN_OUTPUT,
      ...
      spinnerPID.setOutputRange(Constants.COLORWHEEL_MIN_OUTPUT,
          Constants.COLORWHEEL_MAX_OUTPUT_SPIN);
      // convert color wheel rotations to motorshaft rotations 80 shatf rotations = 1 color wheel
      // rotation
      spinnerPID.setReference(spinnerEncoder.getPosition() + rotations * 80,
